...
class LTEConfigurator {
    ...
    /**
     * Generate LTE configuration with multiple providers
     * @param {object} config Configuration with multiple providers
     * @returns {object} Result object with script and success status
     */
    generateMultiProviderLTEConfig(config) {
        try {
            this.validateMultiProviderConfig(config);

            // Transform providers array into apnProfiles format
            const apnProfiles = config.providers.map((provider, index) => ({
                provider: provider.provider || 'custom',
                apnName: provider.apnName,
                username: provider.apnUsername || '',
                password: provider.apnPassword || '',
                profileName: provider.apnProfileName || `lte-profile-${index}`,
                authMethod: config.authMethod || 'chap',
                ipType: config.ipType || 'ipv4',
                routeDistance: provider.routeDistance || (index + 1),
                isMain: index === 0  // First provider is primary
            }));

            const script = this.createMultiProviderScript({
            ...
            console.error('Multi-provider LTE configuration generation failed:', error);
            return {
                script: `# Error generating LTE configuration: ${error.message}`,
                success: false,
                error: error.message
    ...
    /**
     * Validate multi-provider LTE configuration
     * @param {object} config
     */
    validateMultiProviderConfig(config) {
        ...
        config.providers.forEach((provider, index) => {
            if (!provider.apnName) {
                throw new Error(`Provider ${index + 1}: APN name is required`);
            }
        });
    }
    ...
    /**
     * Create MikroTik RouterOS LTE script with multiple APN profiles
     * @param {object} config
     * @returns {string}
     */
    createMultiProviderScript(config) {
        const mainProfile = config.apnProfiles.find(p => p.isMain) || config.apnProfiles[0];
        const mainProviderName = this.providers[mainProfile?.provider]?.name || 'Custom Provider';

        let script = '# MikroTik RouterOS LTE Configuration Script\n';
        script += '# Multi-Provider Configuration with Failover Support\n';
        script += `# Primary Provider: ${mainProviderName}\n`;
        script += `# Total Providers: ${config.apnProfiles.length}\n`;
        script += `# Interface: ${config.lteInterface}\n`;
        script += `# Generated: ${new Date().toLocaleDateString('de-DE')} ${new Date().toLocaleTimeString('de-DE')}\n`;
        script += `# Generated by WireGuard MikroTik Configurator\n\n`;
        ...
        // Step 2: Create APN profiles for all providers
        script += `# Step 2: Create ${config.apnProfiles.length} APN Profile(s)\n`;
        config.apnProfiles.forEach((profile, index) => {
            const providerName = this.providers[profile.provider]?.name || 'Custom Provider';
            script += `# Provider ${index + 1}: ${providerName} (Priority: ${profile.routeDistance})\n`;
        ...
        // Step 3: Set primary APN profile to LTE interface
        script += `# Step 3: Set Primary APN Profile to LTE Interface\n`;
        script += `/interface lte set ${config.lteInterface} apn-profiles="${mainProfile.profileName}"\n\n`;

        // Step 4: Optional - Configure multiple profiles for failover
        if (config.apnProfiles.length > 1) {
            const allProfiles = config.apnProfiles.map(p => p.profileName).join(',');
            script += `# Step 4: Configure Failover with Multiple Profiles\n`;
            script += `# Set all APN profiles for automatic failover\n`;
            script += `/interface lte set ${config.lteInterface} apn-profiles="${allProfiles}"\n\n`;
        }

        // Step 5: Enable the LTE interface
        script += `# Step ${config.apnProfiles.length > 1 ? '5' : '4'}: Enable LTE Interface\n`;
        script += `/interface lte set ${config.lteInterface} disabled=no\n\n`;

        // Step 6: Create default route(s) if enabled
        if (config.setDefaultRoute) {
            script += `# Step ${config.apnProfiles.length > 1 ? '6' : '5'}: Configure Default Routes with Priorities\n`;
            config.apnProfiles.forEach((profile) => {
                script += `# Route via ${this.providers[profile.provider]?.name || profile.profileName} (Distance: ${profile.routeDistance})\n`;
                script += `/ip route add dst-address=0.0.0.0/0 gateway=${config.lteInterface} distance=${profile.routeDistance} comment="${profile.profileName}"\n`;
            });
            script += `\n`;
        }

        // Step 7: Configure NAT if enabled
        if (config.enableLteNat) {
            script += `# Step ${config.setDefaultRoute ? '7' : config.apnProfiles.length > 1 ? '6' : '5'}: Configure NAT for LTE\n`;
            script += `/ip firewall nat add chain=srcnat out-interface=${config.lteInterface} action=masquerade comment="LTE NAT"\n\n`;
        }

        // Step 8: Configure Firewall if enabled
        if (config.enableLteFirewall) {
            const stepNum = (config.enableLteNat ? 8 : (config.setDefaultRoute ? 7 : (config.apnProfiles.length > 1 ? 6 : 5)));
            script += `# Step ${stepNum}: Configure Firewall Rules for LTE\n`;
            script += `# Allow established and related connections\n`;
            script += `/ip firewall filter add chain=input connection-state=established,related action=accept comment="Allow established/related"\n`;
            script += `/ip firewall filter add chain=forward connection-state=established,related action=accept comment="Allow established/related"\n\n`;

            script += `# Allow LAN to WAN\n`;
            script += `/ip firewall filter add chain=forward src-address=${config.localLanNetwork} action=accept comment="Allow LAN to WAN"\n\n`;

            script += `# Drop everything else from WAN\n`;
            script += `/ip firewall filter add chain=input in-interface=${config.lteInterface} action=drop comment="Drop WAN input"\n`;
            script += `/ip firewall filter add chain=forward in-interface=${config.lteInterface} action=drop comment="Drop WAN forward"\n\n`;
        }

        // Step 9: Configure DNS if enabled
        if (config.enableLteDns) {
            const stepNum = (config.enableLteFirewall ? 9 : (config.enableLteNat ? 8 : (config.setDefaultRoute ? 7 : (config.apnProfiles.length > 1 ? 6 : 5))));
            script += `# Step ${stepNum}: Configure DNS Servers\n`;
            const dnsServers = config.lteDnsServers.split(',').map(s => s.trim()).filter(s => s);
            dnsServers.forEach((dns) => {
                script += `/ip dns static add name=dns address=${dns}\n`;
            });
            script += `/ip dns set servers="${dnsServers.join(',')}"\n`;
            script += `/ip dns set allow-remote-requests=yes\n\n`;
        }

        script += `# Configuration complete!\n`;
        script += `# The LTE interface should now be active with ${config.apnProfiles.length} APN profile(s)\n`;
        if (config.apnProfiles.length > 1) {
            script += `# Failover will automatically switch between providers based on route distance\n`;
            script += `# Lower distance values have higher priority\n`;
        }
